...
sudo apt-get -y install wget gcc openssl-devel bzip2-devel libffi-devel liblzma-devel tk-devel zlib-devel
sudo apt-get -y install autoconf automake  binutils bison flex gcc-c++ gdb glibc-devel libtool make pkgconf strace
sudo apt-get -y install byacc ccache cscope ctags elfutils indent ltrace perf valgrind
...
#sudo apt-get -y install python3.13-modules-tkinter
